...
if __name__ == "__main__":
    ...
    print("Initial agent position:", controller.last_event.metadata["agent"]["position"])
    
    # Move the agent ahead slightly with more deliberate steps
    step_size = 0.25
    for i in range(5):
        event = controller.step(action="MoveAhead", moveMagnitude=step_size)
        if not event.metadata["lastActionSuccess"]:
            print(f"Movement blocked: {event.metadata['errorMessage']}")
            controller.step(action="MoveBack", moveMagnitude=step_size)
            controller.step(action="RotateRight", degrees=90)
        time.sleep(1)  # Adding a longer delay to observe the movements
        print("Agent position after move:", controller.last_event.metadata["agent"]["position"])
    
    # Teleport to the object and interact
    try:
        obj_id = tp_to_object("Knife")
        print(f"Teleported to object {obj_id}")
        controller.step(action="LookDown")
        time.sleep(5)  # Adding delay to observe looking down
        controller.step(action="PickupObject", objectId=obj_id, forceAction=True)
        print("Picked up the Knife")
    ...
    print("Objects in the scene:", objects)
