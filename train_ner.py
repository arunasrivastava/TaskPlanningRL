...
training_set = []
span_key = "sc"
base_path = "ARUNASP24/TaskPlanningRL/brat/recipes"

for j in range(100):
    txt_filename = os.path.join(base_path, f"recipe{j}.txt")
    ann_filename = os.path.join(base_path, f"recipe{j}.ann")
    # ARUNASP24/TaskPlanningRL/brat/recipes/recipe0.ann
    # ARUNASP24/TaskPlanningRL/train_ner.py
    # Check if the files exist before opening
    if not os.path.exists(txt_filename) or not os.path.exists(ann_filename):
        print(f"File {txt_filename} or {ann_filename} not found. Skipping.")
        continue
    ...
    for annotation in annotations:
        for i, dot_index in enumerate(dot_indices):
            if int(annotation[3]) <= dot_index:
                if model_type == "span":
                    training_data[i][1]["spans"][span_key].append(
                        (int(annotation[2]) - offsets[i], int(annotation[3]) - offsets[i], annotation[1]))
                else:
                    training_data[i][1]["entities"].append(
                        (int(annotation[2]) - offsets[i], int(annotation[3]) - offsets[i], annotation[1]))
                break
...
if model_type == "span":
    ...
    print({'VERB': ['Refrigerate'], 'WHAT': ['the lettuce'], 'WHERE': [], 'HOW': [], 'TIME': [], 'TEMP': []})
